...
decoders.Int32Decoder = Int32Decoder
decoders.EnumDecoder = EnumDecoder

decoders.Int64Decoder = Int64Decoder

decoders.UInt32Decoder = UInt32Decoder
decoders.UInt64Decoder = UInt64Decoder
decoders.SInt32Decoder = SInt32Decoder
decoders.SInt64Decoder = SInt64Decoder

decoders.Fixed32Decoder  = Fixed32Decoder
decoders.Fixed64Decoder  = Fixed64Decoder
decoders.SFixed32Decoder = SFixed32Decoder
decoders.SFixed64Decoder = SFixed64Decoder
decoders.FloatDecoder = FloatDecoder
decoders.DoubleDecoder = DoubleDecoder
decoders.BoolDecoder = BoolDecoder
...
decoders.StringDecoder = StringDecoder
...
decoders.BytesDecoder = BytesDecoder
...
function _SkipGroup(buffer, pos, pend)
    while pos < pend do
        ...
        new_pos = SkipField(buffer, pos, pend, tag_bytes)
        if new_pos == -1 then
            return pos
        end
        pos = new_pos
