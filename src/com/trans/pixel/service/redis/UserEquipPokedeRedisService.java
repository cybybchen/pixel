...
public class UserEquipPokedeRedisService extends RedisService {
	...
	public void updateUserEquipPokede(final UserEquipPokedeBean userPokede, final long userId) {
		...
		hput(key, "" + userPokede.getItemId(), JSONObject.fromObject(userPokede).toString(), userId);
	...
	public UserEquipPokedeBean selectUserEquipPokede(final long userId, final int itemId) {
		String value = hget(RedisKey.USER_EQUIP_POKEDE_PREFIX + userId, "" + itemId, userId);
		if (value == null)
			return null;
		
		expire(RedisKey.USER_POKEDE_PREFIX + userId, RedisExpiredConst.EXPIRED_USERINFO_7DAY, userId);
	...
	public List<UserEquipPokedeBean> selectUserEquipPokedeList(final long userId) {
		...
		Iterator<Entry<String, String>> it = hget(RedisKey.USER_EQUIP_POKEDE_PREFIX + userId, userId).entrySet().iterator();
		while (it.hasNext()) {
	...
	public void updateUserEquipPokedeList(final List<UserEquipPokedeBean> userPokedeList, final long userId) {
		...
		this.hputAll(RedisKey.USER_EQUIP_POKEDE_PREFIX + userId, map, userId);
		expire(RedisKey.USER_POKEDE_PREFIX + userId, RedisExpiredConst.EXPIRED_USERINFO_7DAY, userId);
	}
	
	public boolean isExistPokedeKey(final long userId) {
		return exists(RedisKey.USER_EQUIP_POKEDE_PREFIX + userId, userId);
	}
