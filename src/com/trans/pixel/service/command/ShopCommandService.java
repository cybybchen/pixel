...
public class ShopCommandService extends BaseCommandService{
	...
	public void DailyShopPurchase(RequestDailyShopPurchaseCommand cmd, Builder responseBuilder, UserBean user){
        ...
        logService.sendShopLog(user.getServerId(), user.getId(), 0, commbuilder.getItemid(), 1, commbuilder.getCurrency(), cost);
...
//			cost = cost*commbuilder.getDiscount()/100;
		if (commbuilder.getPosition() == 3 && user.getFriendVip() == 1) {
			...
			pusher.pushRewardCommand(responseBuilder, user, RewardConst.JEWEL);
			logService.sendShopLog(user.getServerId(), user.getId(), 3, commbuilder.getItemid(), 1, commbuilder.getCurrency(), cost);
		}
...
//		responseBuilder.setBlackShopCommand(shop);
	...
	public void UnionShopPurchase(RequestUnionShopPurchaseCommand cmd, Builder responseBuilder, UserBean user){
		...
		if(commbuilder.getIsOut() || commbuilder.getId() != cmd.getId()){
			...
			pusher.pushRewardCommand(responseBuilder, user, RewardConst.JEWEL);
			logService.sendShopLog(user.getServerId(), user.getId(), 5, commbuilder.getItemid(), 1, commbuilder.getCurrency(), commbuilder.getCost());
		}
	...
	public void RaidShopPurchase(RequestRaidShopPurchaseCommand cmd, Builder responseBuilder, UserBean user){
		...
		if(commbuilder.getIsOut() || commbuilder.getId() != cmd.getId()){
			...
			pusher.pushRewardCommand(responseBuilder, user, RewardConst.JEWEL);
			logService.sendShopLog(user.getServerId(), user.getId(), 2, commbuilder.getItemid(), 1, commbuilder.getCurrency(), commbuilder.getCost());
		}
...
//			}
	...
	public void PVPShopPurchase(RequestPVPShopPurchaseCommand cmd, Builder responseBuilder, UserBean user){
		...
		if((commbuilder.getLimit()+count > commbuilder.getMaxlimit()+othertime && commbuilder.getMaxlimit() >= 0) /*|| commbuilder.getId() != cmd.getId()*/){
			...
			pusher.pushRewardCommand(responseBuilder, user, RewardConst.JEWEL);
			logService.sendShopLog(user.getServerId(), user.getId(), 2, commbuilder.getItemid(), count, commbuilder.getCurrency(), commbuilder.getCost()*count);
		}
...
//		UserRankBean myrank = ladderRedisService.getUserRankByUserId(user.getServerId(), user.getId());
		...
		if((commbuilder.getLimit()+count > commbuilder.getMaxlimit()+othertime && commbuilder.getMaxlimit() >= 0)/* || commbuilder.getId() != cmd.getId()*/){
			...
			pusher.pushRewardCommand(responseBuilder, user, RewardConst.JEWEL);
			logService.sendShopLog(user.getServerId(), user.getId(), 1, commbuilder.getItemid(), count, commbuilder.getCurrency(), commbuilder.getCost()*count);
		}
