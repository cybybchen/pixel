...
import com.trans.pixel.protoc.MohuaProto.MohuaCard;
...
import com.trans.pixel.protoc.MohuaProto.ResponseMohuaUserDataCommand;
...
public class MohuaCommandService extends BaseCommandService {
	...
	public void rewardStage(RequestMohuaStageRewardCommand cmd, Builder responseBuilder, UserBean user) {
		...
		if (rewardList == null || rewardList.size() == 0) {
			logService.sendErrorLog(user.getId(), user.getServerId(), cmd.getClass(), RedisService.formatJson(cmd), ErrorConst.MOHUA_STAGE_REWARD_ERROR);
	...
	public void rewardHp(RequestMohuaHpRewardCommand cmd, Builder responseBuilder, UserBean user) {
		...
		if (rewardList == null || rewardList.size() == 0) {
			logService.sendErrorLog(user.getId(), user.getServerId(), cmd.getClass(), RedisService.formatJson(cmd), ErrorConst.MOHUA_HP_REWARD_ERROR);
